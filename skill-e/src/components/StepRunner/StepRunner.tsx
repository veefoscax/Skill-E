  ...
  return (
    <div className={cn('space-y-3', className)}>
          ...
          {screenshots.length > 1 && (
            <>
              ...
              <Button
                ...
                size="sm"
                onClick={() =>
                  setSelectedIndex(Math.min(screenshots.length - 1, selectedIndex + 1))
                }
      ...
      {currentScreenshot.description && (
        <p className="text-xs text-muted-foreground">{currentScreenshot.description}</p>
      )}
    </div>
  )
}
    ...
    <div className={cn('flex flex-col gap-6 p-6 bg-card rounded-lg border', className)}>
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 space-y-2">
          <div className="flex items-center gap-3">
            <h3 className="text-lg font-semibold">Step {step.stepNumber || step.index + 1}</h3>
            {step.section && <span className="text-sm text-muted-foreground">{step.section}</span>}
      ...
      {showActions && (canRetry || canSkip || canEdit) && (
        <>
          ...
          <div className="flex items-center justify-between">
            <p className="text-sm text-muted-foreground">Choose an action to continue</p>
