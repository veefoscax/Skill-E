...
export function AudioLevelMeter({ audioStream, isActive, className = '' }: AudioLevelMeterProps) {
  ...
  return (
    <div className={`flex flex-col gap-2 ${className}`}>
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <span className="text-sm text-zinc-400">{isActive ? 'Recording' : 'Inactive'}</span>
        {isActive && (
          <span className="text-xs text-zinc-500 ml-auto">{currentLevel.toFixed(0)}%</span>
        )}
      </div>
