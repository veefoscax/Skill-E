...
export function SkillValidator({
  ...
  const [stepStatuses, setStepStatuses] = useState<
    Record<number, 'pending' | 'running' | 'success' | 'error' | 'skipped'>
  >({})
  ...
  const handleStart = useCallback(async () => {
    ...
    setStepStatuses(
      (prev: Record<number, 'pending' | 'running' | 'success' | 'error' | 'skipped'>) => ({
        ...prev,
        0: 'running',
      })
    )
    ...
    try {
      for (let i = 0; i < parsedSkill.steps.length; i++) {
        ...
        if (result.success) {
          ...
          setCurrentError({
            step: parsedSkill.steps[i],
            error: result.error || 'Step requires human intervention',
          })
  ...
  // Handle step fix
  const handleStepFix = useCallback(
    (fixedStep: SkillStep) => {
  ...
  const generateSkillMarkdown = (skill: ParsedSkill): string => {
    ...
    skill.steps.forEach((step, i) => {
      lines.push(
        `### Step ${i + 1}: ${step.instruction.slice(0, 50)}${step.instruction.length > 50 ? '...' : ''}`
      )
      lines.push('')
                ...
                <TooltipContent side="bottom" className="w-64">
                  <div className="space-y-2">
                    <div className="font-semibold">
                      Quality Score: {semanticValidation.score}/100
                    </div>
                    ...
                    >
                      <div className="flex items-start gap-2">
                        <div className="mt-0.5">
                          ...
                          {status === 'skipped' && (
                            <div className="h-4 w-4 rounded-full border-2 border-muted-foreground/50 border-dashed" />
                          )}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="text-sm font-medium truncate">Step {index + 1}</div>
                ...
                {!isExecuting && !executionStats && (
                  <>
                    ...
                    <p className="text-muted-foreground text-center max-w-md mb-6">
                      This skill has {parsedSkill.steps.length} steps. Click Start to begin
                      execution.
                    </p>
                      ...
                      <Card className="mt-8 p-4 max-w-md w-full">
                        ...
                        <div className="space-y-1 text-xs">
                          <div className="flex justify-between">
                            ...
                            <span
                              className={cn(
                                semanticValidation.dimensions.safety >= 90
                                  ? 'text-green-600'
                                  : 'text-muted-foreground'
                              )}
                            >
                          ...
                          <div className="flex justify-between">
                            ...
                            <span
                              className={cn(
                                semanticValidation.dimensions.clarity >= 90
                                  ? 'text-green-600'
                                  : 'text-muted-foreground'
                              )}
                            >
                          ...
                          <div className="flex justify-between">
                            ...
                            <span
                              className={cn(
                                semanticValidation.dimensions.completeness >= 90
                                  ? 'text-green-600'
                                  : 'text-muted-foreground'
                              )}
                            >
                  ...
                  <>
                    ...
                    <div className="text-center mb-6">
                      <p className="text-muted-foreground">
                        Completed {executionStats.completedSteps} of {executionStats.totalSteps}{' '}
                        steps
                      </p>
        ...
        <Dialog open={showSettings} onOpenChange={setShowSettings}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>Configure how the skill validation runs</DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4 py-4">
              <div>
                ...
                <select
                  ...
                  onChange={e =>
                    setExecutionConfig((prev: ExecutionConfig) => ({
                      ...prev,
                      mode: e.target.value as 'dom' | 'image' | 'hybrid',
                    }))
                  }
                >
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={e =>
                    setExecutionConfig((prev: ExecutionConfig) => ({
                      ...prev,
                      pauseOnConfirmation: e.target.checked,
                    }))
                  }
                />
                <label htmlFor="pauseOnConfirmation" className="text-sm">
                  Pause at confirmation points
                </label>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="captureScreenshots" className="text-sm">
                  Capture screenshots
                </label>
              </div>
            </div>
