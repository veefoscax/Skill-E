...
export function ModelSelector({
  ...
  const loadModels = async () => {
    ...
    try {
      ...
      const models = await fetchProviderModels(provider, baseUrl || config.baseUrl, apiKey)
      ...
      if (models.length > 0) {
        ...
        if (!currentIsValid) {
          ...
          console.log(
            `[ModelSelector] Auto-correcting invalid model '${value}' to '${targetModel}'`
          )
        ...
        >
          {loading ? (
            ...
            <option value="" disabled>
              No models found (Check Connection/Key)
            </option>
          )}
        </select>
      </div>
