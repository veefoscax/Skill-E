...
export function DrawingCanvas({
  ...
  const handleFadeComplete = useCallback((drawingId: string) => {
    setDrawings(prev =>
      prev.map(d =>
        d.id === drawingId ? { ...d, fadeState: 'hidden' as const } : d
      )
    );
  }, []);
  ...
  return (
    <div className="relative w-full h-full">
      {/* Pin Mode Indicator */}
      {showIndicator && (
        <PinModeIndicator isPinned={isPinMode} currentColor={currentColor} />
      )}
...
const DrawingElement = memo(function DrawingElement({ drawing, onFadeComplete }: DrawingElementProps) {
