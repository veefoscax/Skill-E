      ...
      <CardContent className="space-y-4">
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="text-xs text-green-800">
              ...
              <p className="opacity-80 mt-1">
                Skill-E runs in user space only. All files (models, logs, sessions) are stored in
                your user profile (%LOCALAPPDATA%).
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="border rounded-md p-3 bg-blue-50/50">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Button variant="outline" size="sm" onClick={openDevTools} className="text-xs">
              <Terminal className="h-3 w-3 mr-1" />
              Open Console
            </Button>
            <Button variant="outline" size="sm" onClick={openLogsFolder} className="text-xs">
        ...
        <div className="border rounded-md p-3">
          ...
          {ollamaStatus.status !== 'idle' && (
            <div
              className={`mt-2 text-xs flex items-start gap-2 ${ollamaStatus.status === 'success' ? 'text-green-600' : 'text-red-600'}`}
            >
        ...
        <div className="border rounded-md p-3">
          ...
          {whisperStatus.status !== 'idle' && (
            <div
              className={`mt-2 text-xs flex items-start gap-2 ${whisperStatus.status === 'success' ? 'text-green-600' : 'text-red-600'}`}
            >
              {whisperStatus.status === 'success' ? (
                <CheckCircle className="h-4 w-4 shrink-0" />
              ) : (
                <XCircle className="h-4 w-4 shrink-0" />
              )}
              <div>
                ...
                {whisperStatus.path && (
                  <p className="mt-1 opacity-80 break-all">Dir: {whisperStatus.path}</p>
                )}
              </div>
            </div>
          )}
          ...
          <Button
            ...
            onClick={async () => {
              try {
                const { invoke } = await import('@tauri-apps/api/core')
                const path =
                  'C:\\Users\\vinif\\AppData\\Local\\Temp\\skill-e-sessions\\session-1769816995636/audio-1769817009486.wav'
