...
function analyzeTextContent(
  ...
  if (
    /^[\d\s\-\(\)\+]{8,}$/.test(text) &&
    (/[\s\(\)\+]/.test(text) || text.split('-').length > 3)
  ) {
    return {
      suggestedName: 'phone',
      type: VariableType.TEXT,
  ...
  if (
    /^[A-Za-z]:\\|^\/|^\.\//.test(text) ||
    /\.(pdf|doc|docx|xls|xlsx|txt|csv|jpg|png)$/i.test(text)
  ) {
    return {
...
function extractFieldNameFromSelector(cssSelector: string, ocrText?: string): string | undefined {
  ...
  const classMatch = cssSelector.match(
    /\.(email|phone|name|address|city|zip|postal|country|username|password|search|query)[\w-]*/i
  )
  if (classMatch) {
