...
import { BrowserCaptureData, ConsoleEntry, NetworkRequest, DOMEvent } from '../browser-capture'
...
function captureNetwork(): void {
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    const requestId = `fetch-${startTime}-${Math.random().toString(36).substr(2, 9)}`
  ...
  XMLHttpRequest.prototype.open = function (
    ...
    return originals.xhrOpen!.apply(this, [method, url, async ?? true, username, password])
  }
  ...
  XMLHttpRequest.prototype.send = function (body?: Document | BodyInit | null) {
    ...
    const requestId = `xhr-${startTime}-${Math.random().toString(36).substr(2, 9)}`
