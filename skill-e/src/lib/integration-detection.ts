...
export async function detectAllIntegrations(): Promise<{
  ...
  const [antigravity, claudeCode] = await Promise.all([detectAntigravity(), detectClaudeCode()])
 ...
 */
export function getManualWorkflowInstructions(integration: 'antigravity' | 'claudeCode'): string[] {
  switch (integration) {
    case 'antigravity':
      return [
