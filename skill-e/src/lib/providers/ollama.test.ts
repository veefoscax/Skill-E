    ...
    it('should work without progress callback', async () => {
      const mockPullResponse = new Response(
        new ReadableStream({
          start(controller) {
            controller.enqueue(
              new TextEncoder().encode('{"status":"success"}\n')
            );
            controller.close();
          },
        }),
      ...
      const mockResponse = new Response(
        new ReadableStream({
          start(controller) {
            // Send partial JSON
            controller.enqueue(
              new TextEncoder().encode('{"message":{"content":"Hel')
            );
            // Complete it in next chunk
            controller.enqueue(
              new TextEncoder().encode('lo"},"done":false}\n')
            );
            controller.enqueue(
              new TextEncoder().encode('{"message":{"content":"!"},"done":true}\n')
            );
    ...
    it('should handle messages without content field', async () => {
      const mockResponse = new Response(
        new ReadableStream({
          start(controller) {
            controller.enqueue(
              new TextEncoder().encode('{"message":{},"done":false}\n')
            );
            controller.enqueue(
              new TextEncoder().encode('{"message":{"content":"Hello"},"done":false}\n')
            );
