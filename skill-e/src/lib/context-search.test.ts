...
describe('Context7Client', () => {
  ...
  describe('searchLibrary', () => {
    ...
    it('should throw error on 401 unauthorized', async () => {
      ...
      await expect(client.searchLibrary('react')).rejects.toThrow(
        'Invalid Context7 API key'
      );
    });
    ...
    it('should throw error on 404 not found', async () => {
      ...
      await expect(client.searchLibrary('unknown')).rejects.toThrow(
        'Library not found'
      );
    });
  });
  ...
  describe('getContext', () => {
    it('should fetch documentation context in JSON format', async () => {
      ...
      const results = await client.getContext(
        '/facebook/react',
    ...
    it('should handle 422 unprocessable entity', async () => {
      ...
      await expect(
        client.getContext('/huge/library', 'query')
      ).rejects.toThrow('Library too large');
    });
  });
  ...
  describe('searchDocumentation', () => {
    it('should search and fetch documentation for a library', async () => {
      ...
      (global.fetch as any).mockResolvedValueOnce({
        ok: true,
        json: async () => [
          { id: '/pandas-dev/pandas', name: 'pandas', version: '2.0.0' },
        ],
      });
...
describe('createContext7Client', () => {
  ...
  it('should throw error for invalid API key format', () => {
    expect(() => createContext7Client('invalid_key')).toThrow(
      'Invalid Context7 API key format'
    );
  });

  it('should throw error for empty API key', () => {
    expect(() => createContext7Client('')).toThrow(
      'Invalid Context7 API key format'
    );
  });
});
