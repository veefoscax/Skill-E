...
function buildContextSection(context: PromptBuildContext): string {
  ...
  const metadata = buildXMLTag('metadata', `
...
`, { number: String(step.number) }))
  ...
  const keySteps = buildXMLTag('key_steps', stepsContent, { count: String(optimizedContext.keySteps.length) });
  
  // Build variables section
  const variablesContent = optimizedContext.variables.length > 0
    ? optimizedContext.variables.map(v => buildXMLTag('variable', `
...
`)).join('\n')
  ...
  const conditionalsContent = optimizedContext.conditionals.length > 0
    ? optimizedContext.conditionals.map(c => buildXMLTag('conditional', `
...
`)).join('\n')
  ...
  return buildXMLTag('recording_context', `
${metadata}
${taskGoal}
${keySteps}
${variables}
...
function buildExampleSection(examples: FewShotExample[]): string {
  const examplesContent = examples.map(ex => buildXMLTag('example', `
${buildXMLTag('name', ex.name)}
${buildXMLTag('description', ex.description)}
${buildXMLTag('input_context', `
...
export function buildPrompt(context: PromptBuildContext): GeneratedPrompt {
  ...
  const sections: PromptSection[] = [
    {
      tag: 'objective',
      content: 'You are an expert at creating Agent Skills from demonstration recordings. Transform the provided recording data into a high-quality SKILL.md following AgentSkills.io specification.',
      priority: 1,
    },
    {
      tag: 'context',
