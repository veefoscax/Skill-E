      ...
      if (options.checkScripts) {
        ...
        scripts.forEach(script => {
          ...
          for (const scriptPattern of config.scripts) {
            totalChecks++;
            if (src.includes(scriptPattern.toLowerCase()) || 
 ...
 * @returns Bot detection result
 */
export async function detectBotMeasures(options?: BotDetectionOptions): Promise<BotDetectionResult> {
...
export function getAntiBotTypeFromUrl(url: string): AntiBotType | undefined {
  ...
  if (urlLower.includes('cloudflare') || urlLower.includes('cf-challenge') || urlLower.includes('cdn-cgi')) {
    return 'cloudflare';
  }
