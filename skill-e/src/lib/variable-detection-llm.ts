...
export function needsLLMEnhancement(
  ...
  const hasLowConfidence = result.variables.some(
...
export async function enhanceWithLLM(
  ...
  try {
    ...
    const mergedVariables = mergeLLMResults(
      preliminaryResult.variables,
      llmResult
    );
...
<user_actions>
${actions.slice(0, 20).map(a => `- ${a.type} at ${formatTime(a.timestamp)}${a.target ? ` on "${a.target}"` : ''}`).join('\n')}
</user_actions>
