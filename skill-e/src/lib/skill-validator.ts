 ...
 */
export function validateSkill(markdown: string, toolDefinition?: ToolDefinition): ValidationResult {
...
function validateToolDefinition(toolDefinition: ToolDefinition): ValidationError[] {
  ...
  if (!/^[a-z][a-z0-9_]*$/.test(toolDefinition.name)) {
    errors.push({
      ...
      fix: `Suggested: "${toolDefinition.name
        .toLowerCase()
        .replace(/[^a-z0-9_]/g, '_')
        .replace(/_+/g, '_')}"`,
    })
  ...
  if (!toolDefinition.input_schema) {
    ...
    // Validate properties
    if (
      !toolDefinition.input_schema.properties ||
      Object.keys(toolDefinition.input_schema.properties).length === 0
    ) {
      errors.push({
        severity: 'info',
...
function checkAtomicScope(markdown: string): ValidationError[] {
  ...
  if (frontmatterMatch) {
    ...
    if (descMatch && descMatch[1].toLowerCase().match(/\band\b.*\band\b/)) {
      errors.push({
        severity: 'info',
        message:
          'Description contains multiple "and" clauses, which may indicate multiple responsibilities',
  ...
  const mainSections = body.match(
    /^##\s+(?!Parameters|When to Use|Prerequisites|Instructions|Verification|Troubleshooting|Notes)(.+)$/gm
  )
