...
const ACTION_PATTERNS = {
  ...
  navigate: /\b(navigate|go to|visit|browse|redirect)\b|(\bopen\b.*\b(page|url|site|website|link))/i,
...
const TARGET_PATTERNS = {
  ...
  elementType: /\b(button|link|input|field|checkbox|radio|dropdown|select|textarea|form|menu|icon|image)\b/gi,
};
...
function extractParameters(markdown: string): SkillParameter[] {
  ...
  const parametersMatch = markdown.match(/## Parameters\s*\n([\s\S]*?)(?=\n##|\n---|\n\*Generated|$)/);
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith('|')) {
      ...
      if (inTable && cells.length >= 4) {
        ...
        let paramName = cells[0].replace(/`/g, '').replace(/[{}]/g, '');
...
function extractPrerequisites(markdown: string): string[] {
  ...
  const prereqMatch = markdown.match(/## Prerequisites\s*\n([\s\S]*?)(?=\n##|\n---|\n\*Generated|$)/);
...
function extractTarget(instruction: string): SkillStep['target'] | undefined {
  ...
  if (!target.description) {
    ...
    if (elementMatches.length > 0) {
      ...
      const relevantWords = words.slice(-2).filter(word => 
        word.length > 2 && !/^(the|a|an|this|that|on|in|at|to|for)$/i.test(word)
      );
 ...
 */
export function addStepFeedback(
  steps: SkillStep[],
  stepId: string,
  feedback: string
): SkillStep[] {
