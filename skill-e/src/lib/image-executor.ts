      ...
      switch (step.actionType) {
        case 'click':
          if (step.target?.coordinates) {
            ...
            return await this.clickAt(step.target.coordinates.x, step.target.coordinates.y, opts)
  ...
  private async matchTemplate(
    ...
    for (let y = 0; y <= screenshot.height - template.height; y += stepSize) {
      for (let x = 0; x <= screenshot.width - template.width; x += stepSize) {
        const score = this.calculateSimilarity(screenshotData, templateData, x, y)
  ...
  private calculateSimilarity(
    ...
    for (let ty = 0; ty < templateHeight; ty += sampleRate) {
      for (let tx = 0; tx < templateWidth; tx += sampleRate) {
        ...
        // Compare RGB values
        const rDiff = Math.abs(templateData.data[templateIdx] - screenshotData.data[screenshotIdx])
