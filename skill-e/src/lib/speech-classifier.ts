...
const PATTERNS = {
  ...
  instruction: {
    prefixes: [
      'click', 'press', 'type', 'enter', 'select', 'choose', 'open',
      'close', 'go to', 'navigate', 'fill', 'submit', 'download',
      'upload', 'scroll', 'refresh', 'clique', 'pressione', 'digite',
      'selecione', 'escolha', 'abra', 'feche', 'vá para', 'navegue',
      'preencha', 'envie', 'baixe', 'faça upload'
    ],
    verbs: [
      'click', 'press', 'type', 'enter', 'select', 'choose', 'open',
      'close', 'navigate', 'fill', 'submit', 'download', 'upload',
      'scroll', 'refresh', 'copy', 'paste', 'delete', 'create',
      'clique', 'pressione', 'digite', 'selecione', 'escolha',
      'abra', 'feche', 'navegue', 'preencha', 'envie'
    ],
  },
  ...
  variable: {
    indicators: [
      'your', 'the user\'s', 'any', 'specific', 'particular',
      'depending on', 'based on', 'enter your', 'type your',
      'input the', 'provide the', 'seu', 'sua', 'do usuário',
      'qualquer', 'específico', 'particular', 'dependendo de',
      'baseado em', 'digite seu', 'digite sua', 'insira o',
      'forneça o'
    ],
    fieldTypes: [
      'name', 'email', 'password', 'username', 'address', 'phone',
      'id', 'code', 'number', 'date', 'time', 'file', 'url',
      'nome', 'e-mail', 'senha', 'usuário', 'endereço', 'telefone',
      'código', 'número', 'data', 'hora', 'arquivo'
    ],
    placeholders: [
      'xxx', '...', '[', ']', '{', '}', '<', '>',
  ...
  conditional: {
    prefixes: [
      'if', 'when', 'in case', 'should', 'unless', 'otherwise',
      'if you', 'if there', 'if it', 'depending on',
      'se', 'quando', 'caso', 'a menos que', 'caso contrário',
      'se você', 'se houver', 'se isto', 'dependendo de'
    ],
    connectors: [
      'then', 'otherwise', 'else', 'or', 'and',
      'então', 'caso contrário', 'senão', 'ou', 'e'
    ],
  },
  ...
  context: {
    phrases: [
      'this is', 'that is', 'we are', 'this means', 'note that',
      'important', 'remember', 'keep in mind', 'as you can see',
      'the reason', 'because', 'therefore', 'esto é', 'isso é',
      'estamos', 'isso significa', 'observe que', 'importante',
      'lembre-se', 'mantenha em mente', 'como você pode ver',
      'a razão', 'porque', 'portanto'
    ],
  },
};
 ...
 */
export function classifySpeechSegments(
...
function detectVariables(
  ...
  const variablePatterns = [
    ...
    // Portuguese patterns
    { regex: /(?:seu|sua|do usuário|digite seu|digite sua)\s+(\w+(?:\s+\w+){0,2})/gi, type: 'text' },
  ...
  for (const fieldType of PATTERNS.variable.fieldTypes) {
    const fieldPattern = new RegExp(
      `(?:your|the|o|a|seu|sua)\\s+${fieldType}\\b`,
      'i'
    );
...
function calculateContextScore(text: string): number {
  ...
  // Explanatory words
  const explanatoryWords = ['because', 'since', 'therefore', 'reason', 'why', 
    'porque', 'desde', 'portanto', 'razão'];
...
function isValidVariableName(name: string): boolean {
  ...
  const stopWords = new Set([
    ...
    'o', 'a', 'os', 'as', 'este', 'esta', 'isto', 'eles', 'elas'
  ]);
...
export function getClassificationStats(
  ...
  return {
    ...
    byType,
    allVariables: allVariables.filter((v, i, arr) => 
      arr.findIndex(t => t.name === v.name) === i
    ),
    allConditionals,
  };
