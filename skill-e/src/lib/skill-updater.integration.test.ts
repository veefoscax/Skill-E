...
describe('Skill Updater Integration', () => {
  ...
  it('should apply fix and update step', async () => {
    ...
    const fixResult = await session.generateStepFix('step-2', 'Use selector #submit', provider)
  ...
  it('should reject invalid fixes', async () => {
    ...
    const warningEntry = timeline.find(e => e.message.includes('warnings') && e.stepId === 'step-2')
  ...
  it('should handle fix generation errors gracefully', async () => {
    ...
    const timeline = session.getTimeline()
    const errorEntry = timeline.find(
      e =>
  ...
  it('should use fixStepAndRetry convenience method', async () => {
    ...
    const success = await session.fixStepAndRetry('step-2', 'Use selector .btn', provider)
  ...
  it('should include categorized error in fix request', async () => {
    ...
    const result = await session.generateStepFix('step-2', 'Button loads after delay', provider)
