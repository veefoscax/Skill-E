 ...
 */
const IGNORED_ROLES = new Set(['generic', 'none', 'presentation', 'separator', 'scrollbar'])
...
function isInteractive(node: CDPAccessibilityNode): boolean {
  ...
  if (node.properties) {
    const clickable = node.properties.find(p => p.name === 'clickable' || p.name === 'focusable')
...
export class AccessibilityTreeGenerator {
  ...
  async findByIndex(
    index: number
  ): Promise<{ role: string; name?: string; center: { x: number; y: number } } | null> {
  ...
  async findByName(
    ...
    return text.interactiveElements.find(e => e.name?.toLowerCase().includes(lowerName)) || null
  }
  ...
  async findByRole(
    ...
    Array<{ role: string; name?: string; center: { x: number; y: number }; index: number }>
  > {
    const text = await this.generateText()
    const lowerRole = role.toLowerCase()

    return text.interactiveElements.filter(e => e.role.toLowerCase() === lowerRole)
  }
