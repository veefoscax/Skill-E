...
describe('Context Optimizer', () => {
  ...
  beforeEach(() => {
    ...
    mockSession = {
      ...
      fullTranscript:
        'Click the login button. Enter your email address. Enter your password. Click submit. Wait for dashboard to load.',
      allAnnotations: {
        clicks: [],
        drawings: [],
        selectedElements: [],
  ...
  describe('optimizeContext', () => {
    ...
    it('should select steps with highest importance scores', async () => {
      ...
      for (const step of optimized.keySteps) {
        const hasContent =
          step.description.length > 0 || step.actions.clicks > 0 || step.actions.annotations > 0
    ...
    it('should maintain chronological order of selected steps', async () => {
      ...
      for (let i = 1; i < optimized.keySteps.length; i++) {
        expect(optimized.keySteps[i].number).toBeGreaterThan(optimized.keySteps[i - 1].number)
      }
    })
