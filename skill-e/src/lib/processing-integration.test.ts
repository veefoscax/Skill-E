...
describe('Processing Pipeline Integration Tests', () => {
  ...
  describe('AC3: Step Detection', () => {
    it('should group frames into logical steps', async () => {
      ...
      // Add pause events
      const pauses = detectVoicePauses(mockTranscription.segments, mockCaptureSession.startTime)
  ...
  describe('NFR-5.1: Processing Time Performance', () => {
    it('should process 2-minute recording in under 30 seconds', async () => {
      ...
      await processSession('test-session', mockCaptureSession, mockTranscription, mockAnnotations)
  ...
  describe('Error Handling', () => {
    ...
    it('should throw error for session without frames', async () => {
      ...
      await expect(
        processSession('test-session', invalidSession, mockTranscription, mockAnnotations)
      ).rejects.toThrow()
    })
