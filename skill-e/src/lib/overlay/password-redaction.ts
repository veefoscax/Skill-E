...
export function detectPasswordField(element: HTMLElement | null): PasswordFieldInfo {
  ...
  if (autocomplete) {
    const passwordAutocompleteValues = ['current-password', 'new-password', 'password']
  ...
  const passwordIdPatterns = ['password', 'passwd', 'pwd', 'pass', 'secret', 'pin', 'passphrase']
  ...
  if (form) {
    ...
    const isLoginForm = loginPatterns.some(
      pattern =>
        formId.includes(pattern) || formName.includes(pattern) || formAction.includes(pattern)
    )
  ...
  const isPasswordField = confidence >= 0.8
...
export class PasswordRedactionManager {
  ...
  getVariableName(): string {
    return this.customVariableName || this.detectionResult?.suggestedVariableName || 'PASSWORD'
  }
